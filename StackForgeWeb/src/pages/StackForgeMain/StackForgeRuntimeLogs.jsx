...
import { faCaretDown, faCircleInfo, faClock, faExclamationTriangle, faRefresh, faSearch } from "@fortawesome/free-solid-svg-icons";
...
const StackForgeRuntimeLogs = () => {
    ...
    const handleStatusFilterChange = (value, checked) => {
        setStatusFilters(prev => (checked ? [...prev, value] : prev.filter(v => v !== value)));
    };
    const handleHostFilterChange = (value, checked) => {
        setHostFilters(prev => (checked ? [...prev, value] : prev.filter(v => v !== value)));
    };
    const handleRequestFilterChange = (value, checked) => {
        setRequestFilters(prev => (checked ? [...prev, value] : prev.filter(v => v !== value)));
    };
        ...
        try {
            const response = await fetch("http://localhost:3000/runtime-logs", {
                ...
                body: JSON.stringify({
                    ...
                    projectID,
                    deploymentID,
                    timePeriod: Object.keys(timeFilters).find(k => timeFilters[k]) || "past_30_mins"
                })
            });
    ...
    const distinctStatuses = Array.from(new Set(logs.map(l => l.status).filter(Boolean)));
    const distinctHosts = Array.from(new Set(logs.map(l => l.host).filter(Boolean)));
    const distinctRequests = Array.from(new Set(logs.map(l => l.runtime_path).filter(Boolean)));
    ...
    const filteredLogs = logs.filter(log => {
        ...
        const statusMatches = statusFilters.length === 0 || statusFilters.includes(log.status);
        const hostMatches = hostFilters.length === 0 || hostFilters.includes(log.host);
        const requestMatches = requestFilters.length === 0 || requestFilters.includes(log.runtime_path);
        ...
        return timeMatches && levelMatches && statusMatches && hostMatches && requestMatches && searchMatches;
    });
                    ...
                    <div className="runtimeLogsTopBar">
                        <div className="runtimeLogsTopBarSearchContainer">
                            <div className="runtimeLogsSearchBarWrapper">
                                ...
                                />
                                <FontAwesomeIcon icon={faCircleInfo} className="runtimeLogsSearchIconSupplement" />
                            </div>
                        </div>
                        <button className="runtimeLogsRefreshButton" onClick={fetchLogs}>
                            <FontAwesomeIcon icon={faRefresh} />
                        </button>
                    </div>
                    <div className="runtimeLogsCellContentWrapper">
                        <div className="runtimeLogsContentSideBar">
                            <label className="runtimeLogsContentFilterHeader" onClick={() => setIsTimelineOpen(prev => !prev)}>
                                <span>
                                ...
                                <FontAwesomeIcon
                                    icon={faCaretDown}
                                    style={{ transform: isTimelineOpen ? "rotate(0deg)" : "rotate(-180deg)", transition: "transform 0.3s ease" }}
                                />
                            </label>
                            {isTimelineOpen && (
                                <>
                                    ...
                                    <div className="runtimeLogsContentCheckboxStack">
                                        <div className="runtimeLogsContentCheckboxWrapper">
                                            <input
                                                ...
                                                onChange={() => handleTimeFilterChange("past_30_mins")}
                                            />
                                            Past 30 Minutes
                                        </div>
                                        <div className="runtimeLogsContentCheckboxWrapper">
                            ...
                            <label className="runtimeLogsContentFilterHeader" onClick={() => setIsLevelOpen(prev => !prev)}>
                                <span>
                                ...
                                <FontAwesomeIcon
                                    icon={faCaretDown}
                                    style={{ transform: isLevelOpen ? "rotate(0deg)" : "rotate(-180deg)", transition: "transform 0.3s ease" }}
                                />
                            </label>
                            {isLevelOpen && (
                                <>
                                    ...
                                    <div className="runtimeLogsContentCheckboxStack">
                                        {["Success", "Warning", "Error"].map(level => (
                                            <div key={level} className="runtimeLogsContentCheckboxWrapper">
                                                <input
                                                    type="checkbox"
                        ...
                        <div className="runtimeLogsContentMainFlex">
                            <div className="runtimeLogsContainer">
                                ...
                                {filteredLogs.map(log => {
                                    ...
                                    const rowBorder =
                                        statusCode >= 400 ? "1px solid rgba(229,75,75,0.2)" :
                                            statusCode >= 300 ? "1px solid rgba(229,180,75,0.2)" :
                                                statusCode >= 200 ? "1px solid rgba(91,186,111,0.2)" :
                                                    "transparent";
                                    const rowBackgroundColor =
                                        statusCode >= 400 ? "rgba(229,75,75,0.05)" :
                                            statusCode >= 300 ? "rgba(229,180,75,0.05)" :
                                                statusCode >= 200 ? "rgba(91,186,111,0.05)" :
                                                    "transparent";
                                    ...
                                    return (
                                        <div
                                            ...
                                            style={{ border: rowBorder, backgroundColor: rowBackgroundColor }}
                                        >
                                            <Tippy content={new Date(log.timestamp).toLocaleTimeString()} theme="tooltip-light" arrow={false}>
                                                <div className="runtimeLogsEntryCellMedium">{new Date(log.timestamp).toLocaleTimeString()}</div>
                                            </Tippy>
                                            <Tippy content={log.status || "—"} theme="tooltip-light" arrow={false}>
                                                <div className="runtimeLogsEntryCellShort" style={{ color: statusColor }}>
                                                    {log.status || "—"}
                                                </div>
                                            </Tippy>
                                            <Tippy content={log.host || "—"} theme="tooltip-light" arrow={false}>
                                                <div className="runtimeLogsEntryCellMedium">{log.host || "—"}</div>
                                            </Tippy>
                                            <Tippy content={log.runtime_path} theme="tooltip-light" arrow={false}>
                                                <div className="runtimeLogsEntryCellMedium">{log.runtime_path}</div>
                                            </Tippy>
                                            <Tippy content={log.runtime_messages} theme="tooltip-light" arrow={false}>
                                                <div className="runtimeLogsEntryCellLong">{log.runtime_messages}</div>
                                            </Tippy>
                                        </div>
                                    );
                                })}
